...
# Try to get API key from .env first, then from Streamlit secrets
EURI_API_KEY = os.getenv("EURI_API_KEY") or st.secrets.get("EURI_API_KEY")
if not EURI_API_KEY:
    st.error("EURI_API_KEY is not set. Add it to .env (local) or Secrets (cloud).")
    st.stop()
    ...
    if uploaded_files:
        ...
        if st.button("🚀 Process Documents", type="primary"):
            with st.spinner("Processing your medical documents..."):
                ...
                #                     font-size:1rem; color:#333; border:1px solid #ddd;">
                #             ✅ Process completed successfully! <br>
                #             Your medical intelligence assitance is ready.
                #         </div>
                #         """,
                #         unsafe_allow_html=True
                #     )
            ...
            # banner animation
            # Create a placeholder for banner
            banner = st.empty()
            ...
            <div style="
                ...
                z-index: 1000;
            ">
            🎉 Your Intelligent Medical Document Assistant is ready to use.
            </div>
            """, unsafe_allow_html=True)
            ...
            # # animation -3
            # # Load a professional animation (from LottieFiles)
            # url = "https://assets10.lottiefiles.com/packages/lf20_j1adxtyb.json"
            # r = requests.get(url)
            # st_lottie(r.json(), height=150, key="done")
...
st.markdown(
    ...
    <div style="text-align: center; color: #666; font-size: 0.9rem; line-height: 1.6;">
        <p>🤖 Powered by <b>Euri AI</b> & <b>LangChain</b> | 🏥 Medical Document Intelligence</p>
        <p>🧑‍💻 Developed by <b>Krishna Aleti</b> | © 2025 All rights reserved</p>
    </div>
    """,
    unsafe_allow_html=True
)
